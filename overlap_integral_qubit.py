...
XYZ_FILE  = sys.argv[1]   # <-- replace with your geometry file
BASIS     = 'def2-SVP'
N_CAS_ORB = 10    # number of active orbitals
N_CAS_EL  = 10   # number of active electrons
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 5b: Optimal Theta from tan formaula
# ─────────────────────────────────────────────────────────────────────────────


def optimal_theta_tan(O_css, O_dx, phi):
    num   = -2.0 * O_css * O_dx * np.cos(phi)
    denom = O_dx**2 - O_css**2
    if abs(denom) < 1e-12:
        return np.pi / 4.0
    two_theta = np.arctan2(num, denom)
    Theta_tan = two_theta / 2.0
    if Theta_tan < 0:
        Theta_tan += np.pi / 2.0
    return Theta_tan
...
# ─────────────────────────────────────────────────────────────────────────────
# Step 5c: CSS and DX pole overlaps + tan formula
# ─────────────────────────────────────────────────────────────────────────────

print(f'\n--- Step 1d: CSS and DX pole overlaps ---')

Ca_css = C.copy(); Cb_css = C.copy()
Ca_dx  = C.copy(); Cb_dx  = C.copy()
Ca_dx[:, hono] = C[:, luno]
Cb_dx[:, hono] = C[:, luno]

O_css = compute_overlap(mol, Ca_css, Cb_css, mc)
O_dx  = compute_overlap(mol, Ca_dx,  Cb_dx,  mc)

if   O_css * O_dx < 0:
    bond_char = 'Covalent/Diradical  (phi* = pi,  Type I  spin-symmetry obstruction)'
elif O_css * O_dx > 0:
    bond_char = 'Ionic/Zwitterionic  (phi* = 0,   Type II configurational obstruction)'
else:
    bond_char = 'Pure Diradical (on Sigma)'

print(f'O_CSS = <CAS|H^2> = {O_css:+.8f}   P = {O_css**2:.6f}')
print(f'O_DX  = <CAS|L^2> = {O_dx:+.8f}   P = {O_dx**2:.6f}')
print(f'sign(O_CSS*O_DX)  = {np.sign(O_css*O_dx):+.0f}')
print(f'Bond character    : {bond_char}')

# Tan formula optimal Theta at phi=0 and phi=pi
Theta_tan_0  = optimal_theta_tan(O_css, O_dx, 0.0)
Theta_tan_pi = optimal_theta_tan(O_css, O_dx, np.pi)
print(f'\nOptimal Theta from tan formula:')
print(f'  phi=0  (ZW): Theta* = {np.degrees(Theta_tan_0):.4f} deg')
print(f'  phi=pi (DR): Theta* = {np.degrees(Theta_tan_pi):.4f} deg')
print(f'  Theta from n_LUNO  : {np.degrees(Theta):.4f} deg')
...
Ca_dx[:, hono] = mf_rhf.mo_coeff[:, luno]
Cb_dx[:, hono] = mf_rhf.mo_coeff[:, luno]
...
def qubit_overlap(O_cs, O_dx, Theta, phi):
    """
    Molecular qubit ansatz

    |Psi> = cos(Theta)|CS>
          + exp(i phi) sin(Theta)|DX>
    O_cs :
        <CAS|H²>
    ...
    real_part = np.cos(Theta)*O_cs + np.cos(phi)*np.sin(Theta)*O_dx
    imag_part = np.sin(phi)*np.sin(Theta)*O_dx
...
phis = np.deg2rad(np.arange(0,361,10))
overlaps_sin=[]
probs_sin=[]
overlaps_tan=[]
probs_tan=[]
...
for phi in phis:
    ...
    Theta_sin = Theta
    Theta_tan = optimal_theta_tan(O_css, O_dx, phi)
    (O1,P1) = qubit_overlap(O_css,O_dx,Theta_sin,phi)
    (O2,P2) = qubit_overlap(O_css,O_dx,Theta_tan,phi)
    overlaps_sin.append(O1)
    probs_sin.append(P1)
    overlaps_tan.append(O2)
    probs_tan.append(P2)


overlaps_sin=np.array(overlaps_sin)
imax_sin=np.argmax(overlaps_sin)
imax_tan=np.argmax(overlaps_tan)
probs_sin=np.array(probs_sin)
best_phi_sin=phis[imax_sin]
best_phi_tan=phis[imax_tan]
best_overlap1=overlaps_sin[imax_sin]
best_prob1=probs_sin[imax_sin]
best_overlap2=overlaps_tan[imax_tan]
best_prob2=probs_tan[imax_tan]

print()

print("="*60)
print("MOLECULAR QUBIT PHASE SCAN")
print("="*60)
print()
print(" Phi(deg)      Overlap")
print("---------------------------")



for p,o in zip(phis,overlaps_tan):
    print(f"{np.degrees(p):8.1f} {o:14.8f}")

print()
print("="*60)
print(f"Optimal Phi (Theta_NO) = {np.degrees(best_phi_sin):.1f} deg")
print(f"Optimal Phi (Theta_OPT) = {np.degrees(best_phi_tan):.1f} deg")
print(f"Maximum overlap (theta from nL) = {best_overlap1:.8f}")
print(f"Maximum probability (theta from nL) = {best_prob1:.8f}")
print(f"Maximum overlap (theta from CAS CI) = {best_overlap2:.8f}")
print(f"Maximum probability (theta from CAS CI) = {best_prob2:.8f}")
print("="*60)
